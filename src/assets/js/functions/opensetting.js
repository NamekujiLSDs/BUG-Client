    ...
    Object.keys(settings[cat]).forEach(function (setting) {
      ...
      switch (settings[cat][setting]["type"]) {
        case "cb": {
          ...
          inp.setAttribute(
            "onclick",
            "window.bugSetting('" + setting + "',this.checked)",
          );
        ...
        case "select": {
          ...
          select.setAttribute(
            "onchange",
            "window.bugSetting('" + setting + "',this.value)",
          );
          Object.keys(settings[cat][setting]["options"]).forEach((option) => {
            ...
            option === settings[cat][setting]["value"]
        ...
        case "file": {
          const pathDisp = Object.assign(document.createElement("div"), {
            ...
            innerText:
              settings[cat][setting]["value"].length > 0 &&
              settings[cat][setting]["value"]
                ? path.basename(settings[cat][setting]["value"])
                : "No selected file...",
          ...
          fileOpener.setAttribute(
            "onclick",
            "window.bugFile('" + setting + "')",
          );
